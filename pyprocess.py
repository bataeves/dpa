  ...
  def _sample_path(self, times, N=1):
    ...
    if not self.conditional:
      ...
      for i, delta in enumerate(deltas):
        x = x * (1 - delta / T) + self.endPosition * delta / T + self.sigma * np.sqrt(
    ...
    if not self.conditional:
      ...
      for i, position in enumerate(xPath):
        ...
        if delta != 0:
          ...
          covXY = self.sigma ** 2 * (1 + np.exp(-2 * self.theta * delta) - 2 * np.exp(-self.theta * delta)) / (
          2 * self.theta ** 2)
          y = muY + np.sqrt(sigmaY - covXY ** 2 / sigmaX) * self.Normal.rvs() + covXY / np.sqrt(sigmaX) * listOfNormals[
            i]
        t = position[0]
        path.append((t, y))
      if returnUO == 0:
...
class SqBessel_process(Diffusion_process):
  ...
  def _transition_pdf(self, x, t, y):
    try:
      return (y / x) ** (0.5 * self.mu) * np.exp(-0.5 * (x + y) / self.nu ** 2 / t) / (0.5 * self.nu ** 2 * t) * iv(
        abs(self.mu), 4 * np.sqrt(x * y) / (self.nu ** 2 * t))
    except AttributeError:
...
class CIR_process(Diffusion_process):
  ...
  def __init__(self, parameters, space_time_constraints):
    ...
    _space_time_constraints['startPosition'] = self._inverse_space_transformation(space_time_constraints['startTime'],
                                                                                  space_time_constraints[
                                                                                    'startPosition'])
    try:
      _space_time_constraints['endPosition'] = self._inverse_space_transformation(space_time_constraints['endTime'],
                                                                                  space_time_constraints['endPosition'])
...
class Jump_Diffusion_process(object):
  ...
  def transition_pdf(self, t, y):
    ...
    if not self.conditional:
      ...
      return self._transition_pdf(self.startPosition, t - self.startTime, y) * self._transition_pdf(y, self.endTime - t,
                                                                                                    self.endPosition) \
             / self._transition_pdf(self.startPosition, self.endTime - self.startTime, self.endPosition)
...
class Inverse_Gaussian_process(Jump_Diffusion_process):
  ...
  def _transition_pdf(self, z, t, x):
    ...
    return self.a * (t - self.startTime) / (2 * pi) * (y) ** (-1.5) * np.np.exp(
      -0.5 * ( (self.a * (t - self.startTime)) ** 2 / y + self.b ** 2 * y ) + self.a * (t - self.startTime) * self.b)
